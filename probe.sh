...
cd ${givenPath} && mkdir httpx && cd httpx

cat ${givenPath}/subdomains/actual_Subdomains.txt | httpx -status-code -title -content-length -follow-redirects -tech-detect | tee httpx_with_details.txt
...
printf "Done with Resolving" | notify --silent
